...
namespace PROYECTO_FINAL
{
    class InsumosDAL
    {
        public static int AgregarInsumos(insumo pInsumo)
        {
            ...
            MySqlCommand _comando = new MySqlCommand(String.Format("select CodInsumo from insumo where CodInsumo={0}", pInsumo.CodInsumo), conexion);
            MySqlDataReader _reader = _comando.ExecuteReader();

            Console.WriteLine("DeEBUG AGREGAR");
            bool flag = true;
            string codigoInsumo = pInsumo.CodInsumo.ToString();
            while (_reader.Read())
            {
                string aux = _reader["CodInsumo"].ToString();
                if (string.Equals(aux, codigoInsumo))
                {
                    flag = false;
                }
                Console.WriteLine(_reader["CodInsumo"].ToString());
            }
