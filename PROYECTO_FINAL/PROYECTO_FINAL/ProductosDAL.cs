...
namespace PROYECTO_FINAL
{
    class ProductosDAL
    {
        public static int AgregarProductos(producto pProducto)
        {
            ...
            MySqlCommand _comando = new MySqlCommand(String.Format("select CodProducto from producto where CodProducto={0}", pProducto.CodProducto), conexion);
            MySqlDataReader _reader = _comando.ExecuteReader();

            Console.WriteLine("DeEBUG AGREGAR");
            bool flag = true;
            string codigoProducto = pProducto.CodProducto.ToString();
            while (_reader.Read())
            {
                string aux = _reader["CodProducto"].ToString();
                if (string.Equals(aux, codigoProducto))
                {
                    flag = false;
                }
                Console.WriteLine(_reader["CodProducto"].ToString());
            }
            if (flag)
            {
